...
public class NextBlockCreator {

	private final static Logger LOGGER = Logger.getLogger(NextBlockCreator.class.getName());

	/**
	 * The node for which the block is being created.
	 */
	private final LocalNodeImpl node;

	/**
	 * The predecessor of the block that must be computed.
	 */
	...
	public NextBlockCreator(LocalNodeImpl node, Block previous) {
		...
		node.signal(node.new BlockDiscoveredEvent(block));
	}

	/**
	 * Yields the block that has been created.
	 * 
	 * @return the block
	 */
	Block getBlock() {
	...
	private void requestDeadlineToEveryMiner() {
		node.forEachMiner(miner -> miner.requestDeadline(scoopNumber, generationSignature, this::onDeadlineComputed));
	}
	...
	private void onDeadlineComputed(Deadline deadline, Miner miner) {
		...
		if (!node.hasMiner(miner))
			...
			if (isLegal(deadline)) {
				...
				node.signal(node.new MinerMisbehaviorEvent(miner));
			}
		}
		else
			LOGGER.info("discarding deadline " + deadline + " since it's larger than the current deadline");
