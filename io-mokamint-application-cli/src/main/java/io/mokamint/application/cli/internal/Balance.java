...
@Command(name = "balance",
	description = "Show the balance of a public key in a remote application.",
	showDefaultValues = true)
public class Balance extends AbstractRpcCommand<RemoteApplication> {
	...
	@Option(names = "--uri", description = "the network URI where the API of the remote application is published", defaultValue = "ws://localhost:8050")
	private URI uri;
	...
	private void body(RemoteApplication remote) throws TimeoutException, InterruptedException, CommandException {
		...
		try {
			Optional<BigInteger> maybeBalance = remote.getBalance(signature, publicKey);
